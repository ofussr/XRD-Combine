...
try:
    ...
    from .services.substrate_compare import (
...
# Compatibility names kept for extensions written against the Tkinter module.
WorkspaceItem = ComparisonItem
Assembly = ComparisonAssembly
_palette = comparison_palette
_axis_label = comparison_axis_label
_axis_uses_degrees = comparison_axis_uses_degrees
...
class SubstrateComparisonPage(ttk.Frame):
    ...
    def __init__(
        ...
        self.button_by_item: dict[WorkspaceItem, tk.Button] = {}
    ...
    def _build(self) -> None:
        ...
        self.notebook.add(self.workspace_tab, text=tr("text.workspace"))
        self.notebook.add(self.gallery_tab, text=tr("text.gallery_results"))
        ...
        ttk.Button(
            controls,
            text=tr("text.load_substrate_folder"),
            command=self.load_substrate_folder,
        ).pack(fill="x", pady=2)
        ttk.Separator(controls, orient="horizontal").pack(fill="x", pady=8)
        ttk.Button(
            controls,
            text=tr("text.open_files"),
            command=self.load_files,
        ).pack(fill="x", pady=2)
        ttk.Button(
            controls,
            text=tr("text.open_folder"),
            command=self.load_folder,
        ).pack(fill="x", pady=2)
        ttk.Separator(controls, orient="horizontal").pack(fill="x", pady=8)
        ttk.Button(
            controls,
            text=tr("text.reset_layout"),
        ...
        self.substrate_heading = tk.Label(
            self.workspace,
            text=tr("text.substrates_right_click_to_clone"),
            bg="#f0f0f0",
            font=("Arial", 10, "bold"),
        )
        ...
        self.file_heading = tk.Label(
            self.workspace,
            text=tr("text.measurements_drag_to_connect"),
            bg="#f0f0f0",
            font=("Arial", 10, "bold"),
        )
        ...
        ttk.Button(
            gallery_controls,
            text=tr("text.load_presets"),
            command=self.load_presets,
        ).pack(side="left", padx=4)
        ttk.Button(
            gallery_controls,
            text=tr("text.save_presets"),
            command=self.save_presets,
        ).pack(side="left", padx=4)
        ttk.Button(
            gallery_controls,
            text=tr("text.update_gallery"),
    ...
    def localize_content(self) -> None:
        ...
        try:
            self.context_menu.entryconfigure(
                0,
                label=tr("text.send_to_viewer"),
            )
            self.context_menu.entryconfigure(
                1,
                label=tr("text.send_to_correction"),
            )
            self.context_menu.entryconfigure(
                3,
                label=localised(
    ...
    def _unique_name(self, wanted: str) -> str:
        return self.comparison_state.unique_name(wanted)

    def _button_for(self, item: WorkspaceItem) -> tk.Button:
        return self.button_by_item[item]

    def _sync_item_widget(self, item: WorkspaceItem) -> None:
        button = self._button_for(item)
        button.configure(
            bg="#2ecc71" if item.active else "#d0d0d0",
            relief="solid" if item.active else "raised",
            bd=2 if item.active else 1,
        )
        button.place(x=item.x, y=item.y)

    def _sync_item_geometry(self, item: WorkspaceItem) -> None:
        button = self._button_for(item)
        item.x = button.winfo_x()
        item.y = button.winfo_y()
        item.width = max(1, button.winfo_width())
        item.height = max(1, button.winfo_height())
    ...
    def _make_card(self, name: str, scan: Scan1D, kind: str) -> WorkspaceItem:
        ...
        self.button_by_item[item] = button
        button.update_idletasks()
        self._sync_item_geometry(item)
    ...
    def _clear_kind(self, kind: str) -> None:
        ...
        for item in removed:
            button = self.button_by_item.pop(item)
            self.item_by_button.pop(button, None)
            button.destroy()
        self._sync_widgets()
        if kind == "substrate":
            self.next_substrate_y = 46
    ...
    def _motion(self, event, button: tk.Button) -> None:
        ...
        self.comparison_state.move_item(item, x, y)

    @staticmethod
    def _overlap(first: WorkspaceItem, second: WorkspaceItem) -> int:
        return ComparisonWorkspace.overlap_area(first, second)
    ...
    def set_item_active(self, item: WorkspaceItem, active: bool) -> None:
    ...
    def _assemblies(self) -> list[Assembly]:
        return self.comparison_state.active_assemblies()
    ...
    def refresh_gallery(self) -> None:
        ...
        if not assemblies:
            ttk.Label(
                self.gallery_frame,
                text=tr("text.no_active_groups_exist_in_the_workspace"),
            ).pack(pady=25)
    ...
    def _preset_for(self, name: str):
        return comparison_preset_for(name, self.presets)

    @staticmethod
    def _transformed_y(values, mode: str):
        return transform_comparison_y(values, mode)
